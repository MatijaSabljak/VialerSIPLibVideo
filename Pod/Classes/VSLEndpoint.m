...
//        param.cap_dev = 0;
//
//        //        pj_status_t status = pjsua_call_set_vid_strm(callId, PJSUA_CALL_VID_STRM_ADD, &param);
//        pj_status_t status = pjsua_call_set_vid_strm(callId, PJSUA_CALL_VID_STRM_START_TRANSMIT, &param);
//
//        // DLog(@"pjsua_call_set_vid_strm status = %i", status);
//        pjsua_vid_dev_set_setting(param.cap_dev, PJMEDIA_VID_DEV_CAP_ORIENTATION, (const void *)PJMEDIA_ORIENT_ROTATE_180DEG, PJ_TRUE);
    ...
    else if (callInfo.state == PJSIP_INV_STATE_EARLY)
    {
        ...
        msg = (event->body.tsx_state.type == PJSIP_EVENT_RX_MSG ?
               event->body.tsx_state.src.rdata->msg_info.msg :
               event->body.tsx_state.src.tdata->msg);
...
//                size.h = 100;
//                size.w = 100;
//                enableVideo(ci, vid_idx, callId, size);
//
//            }
//        } else {
    ...
    if (account) {
        ...
        if (call) {
            ...
            if (vid_idx >= 0) {
                ...
                size.h = 100;
                size.w = 100;
                enableVideo(ci, vid_idx, call_id, size);
            }
        }
    }
}
...
static void enableVideo(pjsua_call_info ci,
                        unsigned med_idx,
...
//        if(!(ci.media[med_idx].dir & PJMEDIA_DIR_DECODING)){
//            pj_status_t status = pjsua_call_set_vid_strm(call_id, PJSUA_CALL_VID_STRM_ADD, NULL);
//            return;
//        }
...
static void onCallMediaEvent(pjsua_call_id call_id,
    ...
    if (event->type == PJMEDIA_EVENT_FMT_CHANGED) {
        ...
        enableVideo(ci, med_idx, call_id, event->data.fmt_changed.new_fmt.det.vid.size);
    }
#endif
}
